...
def fetch_stock_data(ticker, period, interval):
    try:
        ...
        # Use period directly instead of trying to parse it
...
def process_data(data):
    ...
    # If data is empty or does not have a DatetimeIndex, return as-is or handle accordingly
    if not isinstance(data.index, pd.DatetimeIndex):
        if 'Date' in data.columns:
            # Convert the 'Date' column to a datetime type and set it as index
            data['Date'] = pd.to_datetime(data['Date'])
            data.set_index('Date', inplace=True)

    # Only localize or convert timezones if data.index is a proper DatetimeIndex
...
interval_mapping = {
    ...
    '1mo': '60m',  # 60 minute data for 1 month
